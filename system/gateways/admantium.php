<?php
	...
	// Proceess only requests from Admantium IP addresses
	$allowed_ips = array('54.76.248.218','34.252.35.60');
	...
	$offer = $db->QueryFetchArray("SELECT * FROM `completed_offers` WHERE `transaction_id`='".$transaction."' AND `method`='admantium' LIMIT 1");
    ...
    elseif(!empty($user['id']) && empty($offer['id']))
    {
		...
		$db->Query("INSERT INTO `completed_offers` (`user_id`,`transaction_id`,`campaign_id`,`campaign_name`,`user_country`,`user_ip`,`revenue`,`reward`,`method`,`status`,`timestamp`) VALUES ('".$user['id']."','".$transaction."','".$offer_id."','".$name."','".$ucountry."','".$ip_address."','".$payout."','".$reward."','admantium','".$status."','".time()."')");
