<?php
	...
	if(isset($_SESSION['PT_User'])){
		...
		$data = $db->QueryFetchArray("SELECT a.*, c.hash FROM users a LEFT JOIN users_sessions c ON c.uid = a.id WHERE a.id = '".$ses_id."' AND a.disabled = '0' LIMIT 1");
	...
	elseif(isset($_COOKIE['SesToken']))
	{
		...
		{
			$data = $db->QueryFetchArray("SELECT a.*, c.hash FROM users a LEFT JOIN users_sessions c ON c.uid = a.id WHERE a.id = '".$ses_id."' AND c.hash = '".$ses_key."' AND a.disabled = '0' LIMIT 1");
			...
			else
			{
				...
				if($check_activity == 0 && !empty($ip_address))
				{
					$browser = $db->EscapeString($_SERVER['HTTP_USER_AGENT']);
