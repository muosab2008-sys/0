<?php
	...
	$total_pages = $db->QueryGetNumRows("SELECT `id` FROM `completed_offers` WHERE `user_id`='".$data['id']."' AND `status`='1'");
	include(BASE_PATH.'/system/libs/paginator.php');

	$urlPattern = GenerateURL('completed&x=(:num)');
	...
	<div class="col-lg-12">
	  <div class="table-responsive card">
		<table class="table table-striped table-hover table-sm table-responsive-sm m-0">
			...
			<tbody>
				<?php
					$offers = $db->QueryFetchArrayAll("SELECT * FROM `completed_offers` WHERE `user_id`='".$data['id']."' AND `status`='1' ORDER BY `id` DESC LIMIT ".$start.",".$limit);
					...
					foreach($offers as $offer)
					{
						echo '<tr>
							...
							<td class="text-center"><b class="text-success">+'.$offer['reward'].' coins</b></td>
							<td class="text-center"><span class="badge bg-success">Complete</span></td>
