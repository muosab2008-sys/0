<?php
	...
	$total_pages = $db->QueryGetNumRows("SELECT `id` FROM `completed_offers` WHERE `user_id`='".$data['id']."' AND (`status`='2' OR `status`='4')");
	include(BASE_PATH.'/system/libs/paginator.php');

	$urlPattern = GenerateURL('rejected&x=(:num)');
	...
	<div class="col-lg-12">
	  <div class="table-responsive card">
		<table class="table table-striped table-hover table-sm table-responsive-sm m-0">
			...
			<tbody>
				<?php
					$offers = $db->QueryFetchArrayAll("SELECT * FROM `completed_offers` WHERE `user_id`='".$data['id']."' AND (`status`='2' OR `status`='4') ORDER BY `id` DESC LIMIT ".$start.",".$limit);
					...
					foreach($offers as $offer)
					{
						echo '<tr>
							...
							<td class="text-center"><b class="text-warning"><strike>'.$offer['reward'].' coins</strike></b></td>
							<td class="text-center">'.($offer['status'] == 4 ? '<span class="badge bg-warning">Proxy / VPN</span>' : '<span class="badge bg-warning">Rejected</span>').'</td>
